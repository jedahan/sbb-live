...
function App() {
  ...
  const setStartingMMR = (event) => void _setStartingMMR(parseInt(event.target.innerText))
    ...
    <div className="App">
      <header className="App-header">
        ...
        >
          <span>
            <h2>starting</h2>
            <h1 contentEditable={true} onBlur={setStartingMMR}>{startingMMR}</h1>
          </span>
          <span>
            <h2>current</h2>
            <h1>{current}</h1>
...
function Record({ hero, placement, mmr, setMMR }: Game, index: number) {
  const heroUri = `/assets/cards/SBB_HERO_${hero.toUpperCase()}.png`;
  const fontSize = 80;
  const textShadow = "6px 6px 2px black";

  const updateMMR = (event) => void setMMR(parseInt(event.target.innerText))
